...
export default class CreateDiscountUseCase
  implements IUseCase<[ICreateDiscountDTO], ReturnValue<Discount>>
{
  constructor(
    ...
    const discount = await this.repository.create({
      data: {
        ...
        conditions: data.conditions?.length
          ...
          : undefined,
        filterRules:
          data?.filterRules && JSON.parse(JSON.stringify(data?.filterRules)),
      },
    });
